(function () {
  ...
  const copy = {
    zh: {
      ...
      resetFilters: "重置",
      scholarSync: "同步 Google Scholar",
      scholarSyncing: "正在同步…",
      scholarUpdated: "Scholar 已更新",
      scholarNeedSetup: "请先配置 Research Worker",
      citedBy: "Google Scholar 引用"
    },
    en: {
      ...
      resetFilters: "Reset",
      scholarSync: "Sync Google Scholar",
      scholarSyncing: "Syncing…",
      scholarUpdated: "Scholar updated",
      scholarNeedSetup: "Configure Research Worker first",
      citedBy: "Google Scholar citations"
    }
  };
  ...
  function scholarWorkerUrl() {
    ...
    let local = null;
    try { local = JSON.parse(localStorage.getItem("nhw-radar-settings-v3") || "{}"); } catch (err) {}
    return String(local?.workerUrl || cfg.workerUrl || "").replace(/\/$/, "");
  }
  ...
  function addCitationBadge(item, article, lang) {
    ...
    badge.title = article.citedByLink || "Google Scholar";
  }
  ...
  function injectScholarOnly(section, lang, articles, knownTitles) {
    ...
    const label = lang === "zh" ? "Google Scholar 同步发现的新条目" : "New entries found by Google Scholar sync";
    ...
    fresh.forEach((a) => {
      ...
      li.innerHTML = `<span class="pub-index">GS</span><div><span class="pub-title"><a${href}>${String(a.title).replace(/[&<>]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))}</a></span><div class="pub-meta">${String(a.authors || "").replace(/[&<>]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))}<br><em>${String(a.publication || "Google Scholar").replace(/[&<>]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))}</em><div class="badges"></div></div></div>`;
  ...
  async function syncGoogleScholar() {
    ...
    const lang = currentLang();
    if (!base) {
      showToast(copy[lang].scholarNeedSetup);
      return;
    }
    ...
    try {
      ...
      showToast(`${copy[lang].scholarUpdated} · ${data.articles?.length || 0}`);
    } catch (err) {
      showToast(`Scholar: ${err.message || "sync failed"}`);
    } finally {
  ...
  function initScholarCache() {
    ...
    if (base && age > 6 * 3600 * 1000) {
      fetch(base + "/scholar").then((r) => r.ok ? r.json() : null).then((data) => {
        if (!data?.articles?.length) return;
        safeSet(STORAGE_SCHOLAR, JSON.stringify(data));
        applyScholarData(data);
      }).catch(() => {});
