(function () {
  ...
  const STORAGE_SCHOLAR = "hewang-homepage-scholar-v1";
  const pairs = [
    ["hero-zh", "hero-en"],
  ...
  const copy = {
    zh: {
      ...
      scholarSync: "同步论文与引用",
      scholarSyncing: "正在同步…",
      scholarUpdated: "论文与引用已更新",
      scholarNeedSetup: "同步服务暂不可用",
      citedBy: "引用"
    },
    en: {
      ...
      resetFilters: "Reset",
      scholarSync: "Sync publications & citations",
      scholarSyncing: "Syncing…",
      scholarUpdated: "Publications & citations updated",
      scholarNeedSetup: "Sync service unavailable",
      citedBy: "Citations"
    }
  };
  ...
  const publicationBibtex = {
    "fakemark: gradient-guided false watermark claims via robust feature fusion": `@article{Wu2026FakeMark,
  author = {Wu, Yutong and Li, Wenyue and Nie, Hewang and Zhou, Ziqi and Li, Jin and Gong, Yixiao and Lu, Songfeng},
  title = {FakeMark: Gradient-Guided False Watermark Claims via Robust Feature Fusion},
  journal = {Cybersecurity},
  year = {2026},
  volume = {9},
  pages = {217},
  doi = {10.1186/s42400-026-00654-8},
  url = {https://doi.org/10.1186/s42400-026-00654-8}
}`,
  ...
  function enhancePublicationSearch(section, lang) {
    ...
    tools.innerHTML = [
      ...
      `<button class="pub-scholar-sync" type="button">${copy[lang].scholarSync}</button>`,
    ...
    const scholarBtn = tools.querySelector(".pub-scholar-sync");
    if (scholarBtn) scholarBtn.addEventListener("click", syncGoogleScholar);
    section.querySelectorAll(".pub-item").forEach((item) => {
  ...
  function updatePublicationToolsLanguage() {
    document.querySelectorAll(".pub-tools").forEach((tools) => {
      ...
      const scholarBtn = tools.querySelector(".pub-scholar-sync");
      if (scholarBtn && !scholarBtn.disabled) scholarBtn.textContent = copy[lang].scholarSync;
  ...
  function scholarWorkerUrl() {
    const cfg = window.NHW_RESEARCH_CONFIG || {};
    return String(cfg.scholarWorkerUrl || cfg.workerUrl || "").replace(/\/$/, "");
  }

  async function fetchOpenAlexPublications() {
    const cfg = window.NHW_RESEARCH_CONFIG || {};
    const orcid = String(cfg.openAlexOrcid || "0000-0001-6652-4489").trim();
    const url = new URL("https://api.openalex.org/works");
    url.searchParams.set("filter", `authorships.author.orcid:${orcid}`);
    url.searchParams.set("sort", "publication_date:desc");
    url.searchParams.set("per-page", "100");
    if (cfg.openAlexMailto) url.searchParams.set("mailto", cfg.openAlexMailto);
    const r = await fetch(url.toString());
    if (!r.ok) throw new Error(`OpenAlex HTTP ${r.status}`);
    const d = await r.json();
    const articles = (d.results || []).map((w) => ({
      title: w.title || w.display_name || "",
      authors: (w.authorships || []).map((a) => a.author?.display_name).filter(Boolean).join(", "),
      publication: w.primary_location?.source?.display_name || w.type || "OpenAlex",
      year: w.publication_year || (w.publication_date || "").slice(0, 4),
      citedBy: Number(w.cited_by_count || 0),
      citedByLink: w.id || "",
      link: w.doi || w.primary_location?.landing_page_url || w.id || "",
      source: "OpenAlex"
    })).filter((a) => a.title);
    return { articles, updatedAt: new Date().toISOString(), source: "OpenAlex" };
  }

  function normalizeScholarTitle(value) {
    return normalizeText(value).replace(/[^a-z0-9\u4e00-\u9fff]+/g, " ").replace(/\s+/g, " ").trim();
  }

  function addCitationBadge(item, article, lang) {
    if (!item || !article) return;
    let badge = item.querySelector(".badge-scholar-cites");
    const meta = item.querySelector(".pub-meta");
    if (!meta) return;
    let badges = meta.querySelector(".badges");
    if (!badges) {
    ...
    if (!badge) {
      badge = document.createElement(article.citedByLink ? "a" : "span");
      badge.className = "badge badge-scholar-cites";
      if (article.citedByLink) {
        badge.href = article.citedByLink;
        badge.target = "_blank";
        badge.rel = "noopener noreferrer";
      }
      badges.appendChild(badge);
    }
    badge.textContent = `${copy[lang].citedBy}: ${Number(article.citedBy || 0)}`;
    badge.title = `${article.source || "OpenAlex"}${article.citedByLink ? " · " + article.citedByLink : ""}`;
  }

  function injectScholarOnly(section, lang, articles, knownTitles) {
    section.querySelector(".scholar-sync-block")?.remove();
    const fresh = articles.filter((a) => a.title && !knownTitles.has(normalizeScholarTitle(a.title))).slice(0, 30);
    if (!fresh.length) return;
    const block = document.createElement("div");
    block.className = "scholar-sync-block";
    const src = articles[0]?.source || "OpenAlex";
    const label = lang === "zh" ? `${src} 同步发现的新条目` : `New entries found by ${src}`;
    block.innerHTML = `<div class="scholar-sync-head"><strong>${label}</strong><span>${fresh.length}</span></div><ul class="pub-list scholar-sync-list"></ul>`;
    const list = block.querySelector("ul");
    fresh.forEach((a) => {
      const li = document.createElement("li");
      li.className = "pub-item pub-item-scholar";
      li.dataset.year = String(a.year || "");
      li.dataset.level = "";
      const href = a.link ? ` href="${String(a.link).replace(/"/g, "&quot;")}" target="_blank" rel="noopener noreferrer"` : "";
      const mark = (a.source || "OpenAlex") === "Google Scholar" ? "GS" : "OA";
      li.innerHTML = `<span class="pub-index">${mark}</span><div><span class="pub-title"><a${href}>${String(a.title).replace(/[&<>]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))}</a></span><div class="pub-meta">${String(a.authors || "").replace(/[&<>]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))}<br><em>${String(a.publication || a.source || "OpenAlex").replace(/[&<>]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))}</em><div class="badges"></div></div></div>`;
      addCitationBadge(li, a, lang);
      list.appendChild(li);
    });
    const tools = section.querySelector(".pub-tools");
  ...
  function applyScholarData(data) {
    ...
    [[$("publications-zh"), "zh"], [$("publications-en"), "en"]].forEach(([section, lang]) => {
      if (!section) return;
      const items = Array.from(section.querySelectorAll(".pub-item:not(.pub-item-scholar)"));
      const known = new Map(items.map((item) => [normalizeScholarTitle(item.querySelector(".pub-title")?.textContent || ""), item]));
      data.articles.forEach((a) => {
        const key = normalizeScholarTitle(a.title || "");
        let item = known.get(key);
        if (!item && key) {
          item = items.find((x) => {
            const t = normalizeScholarTitle(x.querySelector(".pub-title")?.textContent || "");
            return t.length > 24 && key.length > 24 && (t.includes(key) || key.includes(t));
          });
        }
        if (item) addCitationBadge(item, a, lang);
      });
      injectScholarOnly(section, lang, data.articles, new Set(known.keys()));
      const input = section.querySelector(".pub-search");
      filterPublications(section, input?.value || "");
    });
  }

  async function syncGoogleScholar() {
    const base = scholarWorkerUrl();
    const lang = currentLang();
    const buttons = document.querySelectorAll(".pub-scholar-sync");
    buttons.forEach((b) => { b.disabled = true; b.textContent = copy[lang].scholarSyncing; });
    try {
      let data;
      if (base) {
        const r = await fetch(base + "/scholar");
        if (!r.ok) throw new Error((await r.text()).slice(0, 180));
        data = await r.json();
        (data.articles || []).forEach((a) => { if (!a.source) a.source = "Google Scholar"; });
      } else {
        data = await fetchOpenAlexPublications();
      }
      safeSet(STORAGE_SCHOLAR, JSON.stringify(data));
      applyScholarData(data);
      showToast(`${copy[lang].scholarUpdated} · ${data.articles?.length || 0} · ${data.source || (base ? "Google Scholar" : "OpenAlex")}`);
    } catch (err) {
      showToast(`Sync: ${err.message || "failed"}`);
    } finally {
      buttons.forEach((b) => { b.disabled = false; b.textContent = copy[currentLang()].scholarSync; });
    }
  }

  function initScholarCache() {
    let cached = null;
    try {
      cached = JSON.parse(safeGet(STORAGE_SCHOLAR) || "null");
      if (cached?.articles?.length) applyScholarData(cached);
    } catch (err) {}
    const base = scholarWorkerUrl();
    const age = cached?.updatedAt ? Date.now() - new Date(cached.updatedAt).getTime() : Infinity;
    if (age > 12 * 3600 * 1000) {
      const task = base
        ? fetch(base + "/scholar").then((r) => r.ok ? r.json() : null).then((data) => { if (data?.articles) (data.articles || []).forEach((a) => { if (!a.source) a.source = "Google Scholar"; }); return data; })
